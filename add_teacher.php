...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($check->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO teachers 
            (name, username, password, phone, cnic, address, gender, hire_date, image, class_assigned, campus_id,status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssssis", $name, $username, $password, $phone, $cnic, $address, $gender, $hire_date, $image, $assigned_string, $campus_id, $status);
        $stmt->execute();
    ...
    <div class="card shadow-sm">
      <div class="card-body">
        <div class="mb-4 text-center">
          <h3 class="card-title fw-bold ">➕ Add New Teacher</h3>
        </div><br>
